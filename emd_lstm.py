...
class EMD_LSTM():
    ...
    def load_data(self):
        ...
        with open(Path('./EMD/train.pkl'), 'rb') as f:
            self.train_X = pickle.load(f)
        with open(Path('./EMD/test.pkl'), 'rb') as f:
    ...
    def train_test_loop(self, loop=10, units=10, act_f=0, N_layer=1, lr=0.1, dropout=0.0, recurrent_dropout=0.0, epochs=100, batch_size=160):
        ...
        model train
        '''
        get_best_model = ModelCheckpoint('emd_lstm.h5', monitor='val_loss', save_best_only=True)
        reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=20, mode='auto', verbose=1)
        ...
        print('[HAAR_LSTM] ----------> \tEin: {:.8f} \tEout: {:.8f} \tR^2_in: {:.8f} \tR^2_out: {:.8f}\n'.format(ein_rmse, eout_rmse, ein_r2, eout_r2))
